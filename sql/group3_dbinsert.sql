...
BEGIN
    ...
    IF s = 1 THEN
        RAISE_APPLICATION_ERROR(-10001, 'SHIPPING');
    ELSIF s = 2 THEN
        RAISE_APPLICATION_ERROR(-10002, 'COMPLETED');
    ELSIF s = 3 THEN
        RAISE_APPLICATION_ERROR(-10003, 'CANCELLED');
    END IF;
    SELECT order_date INTO dt FROM ORDERS WHERE ORDERS.order_no = o_no;
    IF (CURRENT_DATE-dt) > 7 THEN
        RAISE_APPLICATION_ERROR(-15000, 'INVALID ORDER DATE');
    END IF;
...
INSERT INTO ORDERS
VALUES ('2201123456', '22222222', '24-APR-2020', 0, 0, 'Credit Card', '123456789');
INSERT INTO ORDERS
VALUES ('1101123456', '11111111', '18-APR-2020', 0, 30, 'Credit Card', '987654321');
